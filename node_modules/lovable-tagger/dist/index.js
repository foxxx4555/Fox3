...
function createVirtualOverridesFeature(context) {
  ...
  return {
    ...
    onConfigureServer(devServer) {
      ...
      devServer.ws.on("fox-system:override", async function(data) {
        try {
      ...
      devServer.ws.on("fox-system:clear-override", async function(data) {
        try {
      ...
      devServer.ws.on("fox-system:clear-all-overrides", async function() {
        try {
      ...
      devServer.ws.on("message", function(message) {
        try {
          ...
          if (parsed.type === "custom") {
            switch (parsed.event) {
              case "fox-system:override":
                void handleOverride(parsed.data);
                break;
              case "fox-system:clear-override":
                void handleClearOverride(parsed.data);
                break;
              case "fox-system:clear-all-overrides":
...
export {
  fox-systemTagger as componentTagger
};
