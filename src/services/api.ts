...
export const api = {
  ...
  async postLoad(loadData: any, userId: string) {
    try {
      const { error } = await supabase
        .from('loads')
        .insert([{ ...loadData, owner_id: userId, status: 'available' }]);
  ...
  async getAvailableLoads() {
    try {
      const { data } = await supabase
        ...
        .select(`
          *,
          owner:profiles!loads_owner_id_fkey(id, full_name, phone)
        `)
        .eq('status', 'available')
        .order('created_at', { ascending: false });
  ...
  async acceptLoad(loadId: string, driverId: string) {
    try {
      ...
      const { error } = await supabase
        ...
        .eq('id', loadId)
        .eq('status', 'available'); // 🔥 حماية من الضغط مرتين
      ...
      const { data } = await supabase
        ...
        .select(`
          *,
          ...
          driver:profiles!loads_driver_id_fkey(*)
        `)
  ...
  async getShipperStats(userId: string) {
    try {
      ...
      return { activeLoads: 0, completedTrips: 0 };
    }
  },
  ...
  async getDriverStats(userId: string) {
    try {
      ...
      return { activeLoads: 0, completedTrips: 0, rating: 0 };
    }
  }
};
