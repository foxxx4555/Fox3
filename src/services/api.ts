...
export const api = {
  ...
  async getAvailableLoads() {
    // جلب الشحنات مع بيانات صاحب الشحنة (الاسم، الهاتف، الصورة) ليراها السائق
  ...
  async getUserLoads(userId: string) {
    // جلب شحنات المستخدم مع بيانات صاحب الشحنة والسائق المرتبط بها
    const { data, error } = await supabase
      .from('loads')
      .select(`
        *,
  ...
  // --- 3. السائقين والشاحنات ---
  async getAvailableDrivers() {
    // جلب السائقين المتاحين في التطبيق ليراهم أصحاب الشحنات
    const { data, error } = await supabase
      .from('profiles')
      .select(`
        id,
  ...
  async getAllLoads() {
    const { data, error } = await supabase
      .from('loads')
      .select('*, owner:profiles!loads_owner_id_fkey(full_name)')
      .order('created_at', { ascending: false });
    if (error) throw error;
    return data;
  },
