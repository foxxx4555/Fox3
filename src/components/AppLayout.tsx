...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 w-full overflow-x-hidden" dir="rtl">
      ...
      <aside className={cn(
        "fixed lg:static inset-y-0 right-0 z-50 w-72 bg-[#0f172a] text-white flex flex-col transition-transform duration-300 shadow-2xl",
        sidebarOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0"
      )}>
        <div className="p-8 flex items-center justify-between border-b border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center shadow-lg"><Truck size={22} className="text-white" /></div>
            <h1 className="font-black text-xl italic tracking-tighter">SAS TRANSPORT</h1>
          </div>
          <Button variant="ghost" size="icon" className="lg:hidden text-white/50" onClick={() => setSidebarOpen(false)}><X size={28} /></Button>
        </div>
        ...
        <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
          {navItems.map((item) => (
            <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)}
              ...
              location.pathname === item.path ? "bg-blue-600 text-white shadow-xl shadow-blue-600/30" : "text-slate-400 hover:bg-white/5 hover:text-white")}
            >
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="h-20 bg-white border-b px-4 flex items-center justify-between shrink-0 z-30">
          <Button variant="ghost" size="icon" className="lg:hidden h-14 w-14 rounded-2xl flex items-center justify-center" onClick={() => setSidebarOpen(true)}>
             <Menu size={36} strokeWidth={2.5} className="text-blue-600" />
          </Button>
          
          <div className="flex items-center gap-3">
            <Popover onOpenChange={(open) => { if(open) markNotifsAsRead(userProfile?.id) }}>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" className="relative hover:bg-slate-100 h-11 w-11 rounded-xl">
                  <Bell size={24} className="text-slate-600" />
                  {notifications.some(n => !n.is_read) && <span className="absolute top-1.5 right-1.5 w-3 h-3 bg-rose-500 rounded-full border-2 border-white" />}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-80 p-0 rounded-[2rem] shadow-2xl border-none overflow-hidden" align="start">
                <div className="p-5 bg-blue-600 text-white font-black text-sm">التنبيهات</div>
                <ScrollArea className="h-80 bg-white">
                  {notifications.length > 0 ? notifications.map(n => (
                    <div key={n.id} className="p-4 border-b flex gap-3 group relative hover:bg-slate-50 transition-colors">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-rose-500 opacity-0 group-hover:opacity-100 absolute left-1 top-1" onClick={(e) => handleDeleteNotif(n.id, e)}><Trash2 size={14}/></Button>
                    </div>
                  )) : <div className="p-16 text-center text-slate-400 italic">لا توجد تنبيهات</div>}
                </ScrollArea>
              </PopoverContent>
            </Popover>
            ...
            <div className="flex items-center gap-2 bg-emerald-50 text-emerald-600 px-4 py-2 rounded-xl border border-emerald-100 shadow-sm">
               ...
               <span className="text-[10px] font-black uppercase tracking-widest">النظام نشط</span>
            </div>
          </div>
        </header>
...
async function markNotifsAsRead(userId: string | undefined) {
  if (!userId) return;
  await supabase.from('notifications').update({ is_read: true }).eq('user_id', userId);
}

// ✅ تم تعديل الأسماء للغة عربية ثابتة لحل مشكلة ReferenceError
